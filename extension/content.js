// ==================== GM API 兼容层（浏览器扩展版） ====================
// 目标：让原油猴脚本的主体代码无需改动即可同步调用这些 GM_* 函数。

const storeCache = {};

// 启动时把 chrome.storage.local 全部读入内存缓存（此后 GM_getValue 可同步读取）
async function gmStoreInit() {
  Object.assign(storeCache, await chrome.storage.local.get(null));
}

function GM_getValue(key, defaultValue) {
  return (key in storeCache) ? storeCache[key] : defaultValue;
}
...
chrome.storage.onChanged.addListener((changes, area) => {
  ...
  for (const [key, { newValue }] of Object.entries(changes)) {
    if (newValue === undefined) {
      ...
      storeCache[key] = newValue;
    }
  }
});

// 跨域请求：交给后台 service worker 代发（host_permissions 可绕过 CORS）
function GM_xmlhttpRequest(options) {
  chrome.runtime.sendMessage({
    ...
    if (chrome.runtime.lastError) {
      if (options.onerror) options.onerror({ error: chrome.runtime.lastError.message });
      return;
    }
    if (response && response.ok) {
      ...
      if (response.responseType === 'json') {
        try {
          result.response = JSON.parse(response.responseText);
        } catch (e) {
          result.response = null;
        }
      }
      if (options.onload) options.onload(result);
    } else if (response && response.timedOut && options.ontimeout) {
...
// 系统通知：支持 GM_notification({title,text,...}) 和 GM_notification(text, title) 两种调用方式
function GM_notification(notification, title) {
  let text;
...
// ==================== 以下为原脚本主体（原样移植，仅按计划做定点修改） ====================
(function() {
	'use strict';

	// ==================== 薄荷清新调色板 ====================
	const MINT = {
		panelBg:        'linear-gradient(135deg, #f4fbf8 0%, #e6f5ef 100%)',
		cardBg:         'rgba(255,255,255,0.72)',
		cardBorder:     'rgba(46,196,165,0.16)',
		textMain:       '#33544a',
		textSub:        '#7fa197',
		textFaint:      '#9db8ae',
		accent:         '#2ec4a5',
		accentDark:     '#26ad92',
		accentGrad:     'linear-gradient(135deg, #3fd3a8 0%, #26ad92 100%)',
		accentSoftBg:   'rgba(46,196,165,0.12)',
		accentBorder:   'rgba(46,196,165,0.28)',
		danger:         '#ff8a8a',
		dangerDark:     '#f27676',
		dangerGrad:     'linear-gradient(135deg, #ff9a9a 0%, #f27676 100%)',
		dangerSoftBg:   'rgba(255,138,138,0.12)',
		warn:           '#f5a95c',
		warnGrad:       'linear-gradient(135deg, #f6b26b 0%, #eda14f 100%)',
		sageGrad:       'linear-gradient(135deg, #8ec9b8 0%, #6fb8a6 100%)',
		inputBg:        '#ffffff',
		inputBorder:    'rgba(46,196,165,0.35)',
		logBg:          'rgba(255,255,255,0.78)',
		shadowSoft:     'rgba(31,169,140,0.16)',
		shadowAccent:   'rgba(46,196,165,0.35)',
		hoverSoft:      'rgba(46,196,165,0.2)'
	};
	...
	function updatePauseButton() {
		...
		if (isPaused) {
			btn.innerHTML = '▶️ 继续';
			btn.style.background = 'linear-gradient(135deg, #3fd3a8 0%, #26ad92 100%)';
		} else {
			btn.innerHTML = '⏸️ 暂停';
			btn.style.background = 'linear-gradient(135deg, #f6b26b 0%, #eda14f 100%)';
		}
	}
	...
	// 已完成通知标志（防止 autoSendIfNeeded 每秒重复检测）
	...
	function updateElementStatus(elementId, status, isSuccess = true) {
		...
		if (statusEl) {
			...
			statusEl.style.color = isSuccess ? MINT.accent : MINT.danger;
		}
	}
	...
	function resetAllGMValues() {
		...
		GM_setValue('singleUserGiveUpDate', '');
	}
	...
	function interceptUserDetailApi() {
		// 原实现（改写页面自身 XHR/fetch）在隔离世界无效，已移至 interceptor.js（MAIN world）。
		// 此处仅注册消息监听，接收拦截器经 window.postMessage 传来的用户列表数据。
		window.addEventListener('message', (event) => {
			if (event.source !== window) return;
			const data = event.data;
			if (!data || data.__dyFireExtMsg__ !== true) return;
			if (data.type === 'USER_LIST') {
				processUserApiResponse(data.data);
			}
		});
	}
	...
	function getNextTargetUser() {
		...
		if (userConfig.multiUserMode === 'random') {
			...
			for (let i = 0; i < allTargetUsers.length; i++) {
				...
				if (!sentUsersToday.includes(user) && !failedUsersToday.includes(user)) {
					nextUser = user;
	...
	function addLog(message, type = 'info') {
		...
		logEntry.style.color = type === 'success' ? MINT.accent : type === 'error' ? MINT.danger : MINT.warn;
		logEntry.style.padding = '5px 0';
		logEntry.style.borderBottom = '1px solid rgba(46,196,165,0.12)';
	...
	function updateFireDaysStatus() {
		...
		if (statusEl) {
			...
			statusEl.style.color = MINT.accent;
		}
	}
	...
	async function executeSendProcess() {
		...
		if (retryCount > userConfig.maxRetryCount) {
			...
			// 单用户模式：达最大重试后今日放弃，避免每秒无限重试
			addHistoryLog(`已达到最大重试次数 (${userConfig.maxRetryCount})，今日停止重试`, 'error');
			GM_setValue('singleUserGiveUpDate', getLocalTodayString());
		...
		if (userConfig.enableTargetUser && allTargetUsers.length > 0) {
			const pageType = getPageType();
			// 每个用户的查找周期开始前重置尝试计数（修复 chat 页跨会话累积导致误判失败）
			GM_setValue('searchAttemptCount', 0);
	...
	function updateStatus(status) {
		...
		if (statusEl) {
			if (status === true) {
				...
				statusEl.style.color = MINT.danger;
			} else if (status === 'sending') {
				statusEl.textContent = '发送中';
				statusEl.style.color = MINT.warn;
			}
		}
	...
	function autoSendIfNeeded() {
		...
		if (userConfig.enableTargetUser && allTargetUsers.length > 0) {
			...
			// 单用户达最大重试后今日放弃，避免每秒无限重试
			if (GM_getValue('singleUserGiveUpDate', '') === today) {
				return;
			}
	...
	function updateUserStatusDisplay() {
		...
		if (!userConfig.enableTargetUser || allTargetUsers.length === 0) {
			...
			if (isSentToday) {
				...
				statusEl.style.color = MINT.textSub;
			}
			return;
		}
	...
	function showUserSelectPanel() {
		...
		userSelectPanel.style.cssText = `
            ...
            width: 500px;
            max-height: 80vh;
            background: ${MINT.panelBg};
            border-radius: 20px;
            box-shadow: 0 20px 60px ${MINT.shadowSoft};
            z-index: 10000;
            padding: 0;
		...
		userSelectPanel.innerHTML = `
            <div id="dy-fire-user-select-header" style="padding: 20px; border-bottom: 1px solid ${MINT.cardBorder}; background: rgba(46,196,165,0.06); cursor: move; flex-shrink: 0;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <h3 id="dy-fire-user-select-title" style="margin: 0; color: ${MINT.textMain}; font-size: 18px; font-weight: 600;">👥 选择用户 <span id="dy-fire-user-select-count" style="color:${MINT.accent};">(加载中…)</span></h3>
                    <button id="dy-fire-user-select-close" style="background: ${MINT.accentSoftBg}; border: none; width: 32px; height: 32px; border-radius: 50%; cursor: pointer; color: ${MINT.textMain}; font-size: 18px; display: flex; align-items: center; justify-content: center; transition: all 0.2s ease;">×</button>
                </div>
            </div>

            <div style="padding: 12px 20px; border-bottom: 1px solid ${MINT.cardBorder}; flex-shrink: 0; display: flex; justify-content: space-between;">
                <button id="dy-fire-select-all" style="padding: 8px 16px; background: ${MINT.accentSoftBg}; color: ${MINT.accentDark}; border: 1px solid ${MINT.accentBorder}; border-radius: 8px; cursor: pointer; font-weight: 500; font-size: 13px; transition: all 0.2s ease;">全选</button>
                <span id="dy-fire-user-select-loading-badge" style="font-size: 12px; color: ${MINT.danger}; align-self: center;">⏳ 滚动加载中…</span>
                <button id="dy-fire-deselect-all" style="padding: 8px 16px; background: ${MINT.dangerSoftBg}; color: ${MINT.dangerDark}; border: 1px solid rgba(255,138,138,0.3); border-radius: 8px; cursor: pointer; font-weight: 500; font-size: 13px; transition: all 0.2s ease;">取消全选</button>
            </div>

            <div id="dy-fire-user-list-container" style="flex: 1; overflow-y: auto; background: ${MINT.logBg}; border: 1px solid ${MINT.cardBorder}; margin: 10px 20px; border-radius: 8px; padding: 6px; min-height: 200px; max-height: 320px;"></div>

            <div style="padding: 16px 20px; border-top: 1px solid ${MINT.cardBorder}; background: rgba(46,196,165,0.06); flex-shrink: 0;">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                    <button id="dy-fire-user-select-add" style="padding: 12px; background: ${MINT.accentGrad}; color: white; border: none; border-radius: 10px; cursor: pointer; font-weight: 600; font-size: 14px; transition: all 0.2s ease;">✅ 更新目标用户</button>
                    <button id="dy-fire-user-select-cancel" style="padding: 12px; background: ${MINT.dangerGrad}; color: white; border: none; border-radius: 10px; cursor: pointer; font-weight: 600; font-size: 14px; transition: all 0.2s ease;">❌ 取消</button>
                </div>
            </div>
        `;
		...
		const appendRows = (nicknames) => {
			...
			const hasApiData = Object.keys(userApiMap).length > 0;
			const frag = document.createDocumentFragment();
			nicknames.forEach(nickname => {
				const userId = n2id[nickname];
				const isGroup = hasApiData && !userId;
				...
				const badge = isGroup
					? `<span style="font-size: 11px; color: ${MINT.textSub}; margin-left: 8px; padding: 2px 6px; background: ${MINT.accentSoftBg}; border-radius: 4px;">群聊</span>`
					: '';
				const row = document.createElement('div');
				row.style.cssText = 'padding: 10px; border-bottom: 1px solid rgba(46,196,165,0.12);';
				row.innerHTML = `
                    <label style="display: flex; align-items: center; cursor: pointer;">
                        ...
                        <span style="color: #33544a; font-size: 14px;">${safeNickname}</span>${badge}
                    </label>`;
		...
		closeBtn.addEventListener('mouseenter', () => { closeBtn.style.background = 'rgba(46,196,165,0.2)'; closeBtn.style.transform = 'scale(1.1)'; });
		closeBtn.addEventListener('mouseleave', () => { closeBtn.style.background = 'rgba(46,196,165,0.12)'; closeBtn.style.transform = 'scale(1)'; });
	...
	function createControlPanel() {
		...
		panel.style.cssText = `
            ...
            background: ${MINT.panelBg};
            border-radius: 18px;
            box-shadow: 0 8px 32px ${MINT.shadowSoft}, 0 0 0 1px ${MINT.cardBorder};
            z-index: 9999;
            font-family: 'PingFang SC', 'Microsoft YaHei', sans-serif;
            padding: 0;
            color: ${MINT.textMain};
		...
		panel.innerHTML = `
            <div id="dy-fire-header" style="padding: 20px 20px 15px 20px; border-bottom: 1px solid ${MINT.cardBorder}; cursor: move;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                    <h3 style="margin: 0; color: ${MINT.textMain}; font-size: 18px; display: flex; align-items: center; font-weight: 600;">
                        <span style="display: inline-block; width: 8px; height: 8px; border-radius: 50%; background: ${MINT.accent}; margin-right: 10px; box-shadow: 0 0 8px ${MINT.accent};"></span>
                        🔥 抖音续火助手
                    </h3>
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <div id="dy-fire-pause-status" style="font-size: 12px; color: ${MINT.warn}; font-weight: 500; display: none;">⏸️ 已暂停</div>
                        <button id="dy-fire-helper-close" style="background: ${MINT.accentSoftBg}; border: none; width: 28px; height: 28px; border-radius: 50%; cursor: pointer; color: ${MINT.textMain}; font-size: 16px; display: flex; align-items: center; justify-content: center; transition: all 0.2s ease;">×</button>
                    </div>
                </div>
               
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-size: 12px;">
                    <div style="background: ${MINT.cardBg}; padding: 8px; border-radius: 8px; border: 1px solid ${MINT.cardBorder};">
                        <div style="color: ${MINT.textSub}; margin-bottom: 4px;">今日状态</div>
                        <div id="dy-fire-status" style="color: ${MINT.accent}; font-weight: 600; font-size: 13px;">已发送</div>
                    </div>
                    <div style="background: ${MINT.cardBg}; padding: 8px; border-radius: 8px; border: 1px solid ${MINT.cardBorder};">
                        <div style="color: ${MINT.textSub}; margin-bottom: 4px;">用户状态</div>
                        <div id="dy-fire-user-status" style="color: ${MINT.textSub}; font-weight: 600; font-size: 13px;">${userConfig.enableTargetUser ? '已启用' : '未启用'}</div>
                    </div>
                    <div style="background: ${MINT.cardBg}; padding: 8px; border-radius: 8px; border: 1px solid ${MINT.cardBorder};">
                        <div style="color: ${MINT.textSub}; margin-bottom: 4px;">发送进度</div>
                        <div id="dy-fire-user-progress" style="color: ${MINT.danger}; font-weight: 600; font-size: 13px;"></div>
                    </div>
                    <div style="background: ${MINT.cardBg}; padding: 8px; border-radius: 8px; border: 1px solid ${MINT.cardBorder};">
                        <div style="color: ${MINT.textSub}; margin-bottom: 4px;">重试次数</div>
                        <div id="dy-fire-retry" style="color: ${MINT.textMain}; font-weight: 600; font-size: 13px;">${retryCount}/${userConfig.maxRetryCount}</div>
                    </div>
                </div>
            </div>
           
            <div style="padding: 15px 20px; border-bottom: 1px solid ${MINT.cardBorder};">
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; margin-bottom: 12px;">
                    <div style="background: ${MINT.cardBg}; padding: 8px; border-radius: 8px; border: 1px solid ${MINT.cardBorder};">
                        <div style="color: ${MINT.textSub}; font-size: 11px; margin-bottom: 2px;">下次发送</div>
                        <div id="dy-fire-next" style="color: ${MINT.textMain}; font-size: 12px; font-weight: 500;">2023-11-05 00:01:00</div>
                    </div>
                    <div style="background: ${MINT.cardBg}; padding: 8px; border-radius: 8px; border: 1px solid ${MINT.cardBorder};">
                        <div style="color: ${MINT.textSub}; font-size: 11px; margin-bottom: 2px;">倒计时</div>
                        <div id="dy-fire-countdown" style="color: ${MINT.danger}; font-size: 12px; font-weight: 700;">23:45:12</div>
                    </div>
                    <div style="background: ${MINT.cardBg}; padding: 8px; border-radius: 8px; border: 1px solid ${MINT.cardBorder};">
                        <div style="color: ${MINT.textSub}; font-size: 11px; margin-bottom: 2px;">火花天数</div>
                        <div id="dy-fire-days" style="color: ${MINT.accent}; font-size: 12px; font-weight: 700;">${userConfig.fireDays}</div>
                    </div>
                </div>
               
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px;">
                    <div style="background: ${MINT.cardBg}; padding: 8px; border-radius: 8px; border: 1px solid ${MINT.cardBorder};">
                        <div style="color: ${MINT.textSub}; font-size: 11px; margin-bottom: 2px;">一言状态</div>
                        <div id="dy-fire-hitokoto" style="color: ${MINT.accent}; font-size: 12px; font-weight: 500;">未获取</div>
                    </div>
                    <div style="background: ${MINT.cardBg}; padding: 8px; border-radius: 8px; border: 1px solid ${MINT.cardBorder};">
                        <div style="color: ${MINT.textSub}; font-size: 11px; margin-bottom: 2px;">专属一言状态</div>
                        <div id="dy-fire-special-hitokoto" style="color: ${MINT.accent}; font-size: 12px; font-weight: 500;">未获取</div>
                    </div>
                    <div style="background: ${MINT.cardBg}; padding: 8px; border-radius: 8px; border: 1px solid ${MINT.cardBorder};">
                        <div style="color: ${MINT.textSub}; font-size: 11px; margin-bottom: 2px;">TXTAPI状态</div>
                        <div id="dy-fire-txtapi" style="color: ${MINT.accent}; font-size: 12px; font-weight: 500;">未获取</div>
                    </div>
                </div>
            </div>
...
<div style="padding: 15px 20px; border-bottom: 1px solid ${MINT.cardBorder};">
    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; margin-bottom: 10px;">
        <button id="dy-fire-send" style="padding: 12px; background: ${MINT.accentGrad}; color: white; border: none; border-radius: 10px; cursor: pointer; font-weight: 600; font-size: 14px; transition: all 0.2s ease; box-shadow: 0 4px 12px ${MINT.shadowAccent};">
            🚀 立即发送
        </button>
        <button id="dy-fire-pause" style="padding: 12px; background: ${MINT.warnGrad}; color: white; border: none; border-radius: 10px; cursor: pointer; font-weight: 600; font-size: 14px; transition: all 0.2s ease;">
            ⏸️ 暂停
        </button>
        <button id="dy-fire-reset-users" style="padding: 12px; background: ${MINT.sageGrad}; color: white; border: none; border-radius: 10px; cursor: pointer; font-weight: 600; font-size: 14px; transition: all 0.2s ease;">
            🔄 重置记录
        </button>
    </div>
   
    <div style="display: flex; flex-wrap: wrap; gap: 8px; justify-content: center;">
        <button id="dy-fire-settings" style="flex: 1 0 calc(25% - 6px); min-width: 80px; padding: 10px; background: ${MINT.accentSoftBg}; color: #2c6b5d; border: none; border-radius: 8px; cursor: pointer; font-weight: 500; font-size: 13px; transition: all 0.2s ease; text-align: center;">
            ⚙️ 设置面板
        </button>
        <button id="dy-fire-history" style="flex: 1 0 calc(25% - 6px); min-width: 80px; padding: 10px; background: ${MINT.accentSoftBg}; color: #2c6b5d; border: none; border-radius: 8px; cursor: pointer; font-weight: 500; font-size: 13px; transition: all 0.2s ease; text-align: center;">
            📋 历史日志
        </button>
        <button id="dy-fire-modify-days" style="flex: 1 0 calc(25% - 6px); min-width: 80px; padding: 10px; background: ${MINT.accentSoftBg}; color: #2c6b5d; border: none; border-radius: 8px; cursor: pointer; font-weight: 500; font-size: 13px; transition: all 0.2s ease; text-align: center;">
            📅 火花天数
        </button>
        <button id="dy-fire-clear-days" style="flex: 1 0 calc(25% - 6px); min-width: 80px; padding: 10px; background: ${MINT.dangerSoftBg}; color: ${MINT.dangerDark}; border: none; border-radius: 8px; cursor: pointer; font-weight: 500; font-size: 13px; transition: all 0.2s ease; text-align: center;">
            🗑️ 清空天数
        </button>
        <button id="dy-fire-select-users" style="flex: 1 0 calc(25% - 6px); min-width: 80px; padding: 10px; background: ${MINT.accentSoftBg}; color: #2c6b5d; border: none; border-radius: 8px; cursor: pointer; font-weight: 500; font-size: 13px; transition: all 0.2s ease; text-align: center;">
            👥 用户选择
        </button>
        <button id="dy-fire-reset-retry" style="flex: 1 0 calc(25% - 6px); min-width: 80px; padding: 10px; background: ${MINT.accentSoftBg}; color: #2c6b5d; border: none; border-radius: 8px; cursor: pointer; font-weight: 500; font-size: 13px; transition: all 0.2s ease; text-align: center;">
            🔄 重置重试
        </button>
        <button id="dy-fire-clear" style="flex: 1 0 calc(25% - 6px); min-width: 80px; padding: 10px; background: ${MINT.dangerSoftBg}; color: ${MINT.dangerDark}; border: none; border-radius: 8px; cursor: pointer; font-weight: 500; font-size: 13px; transition: all 0.2s ease; text-align: center;">
            🗑️ 清空数据
        </button>
        <button id="dy-fire-reset" style="flex: 1 0 calc(25% - 6px); min-width: 80px; padding: 10px; background: ${MINT.dangerSoftBg}; color: ${MINT.dangerDark}; border: none; border-radius: 8px; cursor: pointer; font-weight: 500; font-size: 13px; transition: all 0.2s ease; text-align: center;">
            🔧 重置配置
        </button>
    </div>
</div>
            ...
            <div style="padding: 15px 20px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                    ...
                    <div style="font-size: 11px; color: ${MINT.textSub};">实时更新</div>
                </div>
                <div id="dy-fire-log" style="font-size: 12px; height: 120px; overflow-y: auto; line-height: 1.4; background: ${MINT.logBg}; border: 1px solid ${MINT.cardBorder}; border-radius: 8px; padding: 10px;">
                    <div style="color: ${MINT.accent}; padding: 5px 0;">系统已就绪，等待执行...</div>
                </div>
            </div>
        `;
	...
	function addButtonHoverEffects() {
		...
		buttons.forEach(button => {
			if (button.id !== 'dy-fire-helper-close') {
				button.addEventListener('mouseenter', function() {
					...
					this.style.boxShadow = `0 6px 20px ${MINT.shadowAccent}`;
				});
		...
		if (closeBtn) {
			closeBtn.addEventListener('mouseenter', function() {
				this.style.background = MINT.hoverSoft;
				this.style.transform = 'scale(1.1)';
			});

			closeBtn.addEventListener('mouseleave', function() {
				this.style.background = MINT.accentSoftBg;
	...
	function createReopenButton() {
		...
		reopenBtn.style.cssText = `
            ...
            cursor: pointer;
            z-index: 9998;
            box-shadow: 0 6px 20px ${MINT.shadowAccent};
            font-size: 20px;
            font-weight: bold;
            transition: all 0.3s ease;
            backdrop-filter: blur(10px);
		...
		reopenBtn.addEventListener('mouseenter', function() {
			...
			this.style.boxShadow = `0 8px 25px ${MINT.shadowAccent}`;
		});
		...
		// 悬浮球拖拽：复用共享拖拽系统，位移超过阈值视为拖拽而非点击
		addDragFunctionality(reopenBtn, reopenBtn);

		let dragStartX = 0;
		let dragStartY = 0;

		reopenBtn.addEventListener('mousedown', function(e) {
			if (e.button !== 0) return;
			dragStartX = e.clientX;
			dragStartY = e.clientY;
			suppressReopenClick = false;
			this.style.transform = 'scale(1) rotate(0)';
		});

		reopenBtn.addEventListener('mousemove', function(e) {
			if (!isDragging || currentPanel !== reopenBtn) return;
			if (Math.abs(e.clientX - dragStartX) > 4 || Math.abs(e.clientY - dragStartY) > 4) {
				suppressReopenClick = true;
			}
		});

		reopenBtn.addEventListener('mouseup', function() {
			setTimeout(function() {
				suppressReopenClick = false;
			}, 0);
		});
	...
	function showHistoryPanel() {
		...
		const logItems = logs.map(log => `
            <div style="padding: 12px; border-bottom: 1px solid ${MINT.cardBorder}; transition: background 0.2s ease;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
                    <span style="font-size: 11px; color: ${MINT.textSub};">
                        ${new Date(log.timestamp).toLocaleString()}
                    </span>
                    <span style="padding: 4px 8px; border-radius: 12px; font-size: 10px; font-weight: 600;
                         background: ${log.type === 'success' ? 'rgba(46, 196, 165, 0.14)' : log.type === 'error' ? 'rgba(255, 138, 138, 0.14)' : 'rgba(245, 169, 92, 0.16)'}; 
                         color: ${log.type === 'success' ? MINT.accentDark : log.type === 'error' ? MINT.dangerDark : MINT.warn};
                         border: 1px solid ${log.type === 'success' ? 'rgba(46, 196, 165, 0.3)' : log.type === 'error' ? 'rgba(255, 138, 138, 0.3)' : 'rgba(245, 169, 92, 0.3)'}">
                        ${log.type.toUpperCase()}
                    </span>
                </div>
                <div style="font-size: 13px; color: ${MINT.textMain}; line-height: 1.4;">${log.message}</div>
            </div>
        `).join('');

		historyPanel.innerHTML = `
            <div id="dy-fire-history-header" style="padding: 20px; border-bottom: 1px solid ${MINT.cardBorder}; background: rgba(46,196,165,0.06); cursor: move;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <h3 style="margin: 0; color: ${MINT.textMain}; font-size: 18px; font-weight: 600;">
                        📋 历史日志 (${logs.length}/${userConfig.maxHistoryLogs})
                    </h3>
                    <button id="dy-fire-history-close" style="background: ${MINT.accentSoftBg}; border: none; width: 32px; height: 32px; border-radius: 50%; cursor: pointer; color: ${MINT.textMain}; font-size: 18px; display: flex; align-items: center; justify-content: center; transition: all 0.2s ease;">×</button>
                </div>
            </div>
            
            <div style="flex: 1; overflow-y: auto; padding: 0;">
                <div style="min-height: 100%; background: rgba(46,196,165,0.04);">
                    ${logs.length > 0 ? logItems : `
                        <div style="text-align: center; color: ${MINT.textSub}; padding: 60px 20px;">
                            <div style="font-size: 48px; margin-bottom: 20px;">📝</div>
                            <div style="font-size: 16px; color: ${MINT.textSub};">暂无日志记录</div>
                        </div>
                    `}
                </div>
            </div>
            
            <div style="padding: 20px; border-top: 1px solid ${MINT.cardBorder}; background: rgba(46,196,165,0.06);">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                    <button id="dy-fire-history-export" style="padding: 12px; background: ${MINT.accentGrad}; color: white; border: none; border-radius: 10px; cursor: pointer; font-weight: 600; font-size: 14px; transition: all 0.2s ease;">
                        📤 导出日志
                    </button>
                    <button id="dy-fire-history-clear" style="padding: 12px; background: ${MINT.dangerGrad}; color: white; border: none; border-radius: 10px; cursor: pointer; font-weight: 600; font-size: 14px; transition: all 0.2s ease;">
                        🗑️ 清空日志
                    </button>
                </div>
            </div>
    ...
    <div id="dy-fire-settings-header" style="padding: 20px; border-bottom: 1px solid ${MINT.cardBorder}; background: rgba(46,196,165,0.06); cursor: move;">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <h3 style="margin: 0; color: ${MINT.textMain}; font-size: 18px; font-weight: 600;">⚙️ 设置面板</h3>
            <button id="dy-fire-settings-close" style="background: ${MINT.accentSoftBg}; border: none; width: 32px; height: 32px; border-radius: 50%; cursor: pointer; color: ${MINT.textMain}; font-size: 18px; display: flex; align-items: center; justify-content: center; transition: all 0.2s ease;">×</button>
        </div>
    </div>
   
    <div style="display: flex; height: calc(85vh - 140px);">
        <div style="width: 200px; background: rgba(46,196,165,0.05); padding: 20px 0; border-right: 1px solid ${MINT.cardBorder};">
        ...
        <div style="flex: 1; overflow-y: auto; padding: 20px;">
            <div id="basic-settings" class="settings-tab active">
                <div class="settings-section">
                    <h4 style="color: #33544a; margin: 0 0 15px 0; font-size: 16px; font-weight: 600;">🕒 发送时间设置</h4>
                    <div style="margin-bottom: 15px;">
                        <label style="display: flex; align-items: center; cursor: pointer; margin-bottom: 10px;">
                            ...
                            <span style="color: #7fa197;">启用随机发送时间</span>
                        </label>
                    </div>
                    
                    <div id="fixed-time-container" style="margin-bottom: 15px; ${userConfig.sendTimeRandom ? 'display: none;' : ''}">
                        <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">发送时间 (HH:mm:ss)</label>
                        <input type="text" id="dy-fire-settings-time" value="${userConfig.sendTime}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;" placeholder="例如: 00:01:00">
                        <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">设置每日自动发送消息的时间</div>
                    </div>
                    
                    <div id="random-time-container" style="margin-bottom: 15px; ${userConfig.sendTimeRandom ? '' : 'display: none;'}">
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                            <div>
                                <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">开始时间 (HH:mm:ss)</label>
                                <input type="text" id="dy-fire-settings-time-start" value="${userConfig.sendTimeRangeStart}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;" placeholder="例如: 23:30:00">
                            </div>
                            <div>
                                <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">结束时间 (HH:mm:ss)</label>
                                <input type="text" id="dy-fire-settings-time-end" value="${userConfig.sendTimeRangeEnd}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;" placeholder="例如: 00:30:00">
                            </div>
                        </div>
                        <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">在开始时间和结束时间之间随机选择一个时间发送（支持跨天）</div>
                    </div>
                </div>

                <div class="settings-section">
                    <h4 style="color: #33544a; margin: 0 0 15px 0; font-size: 16px; font-weight: 600;">⏱️ 启动设置</h4>
                    <div style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">页面初始加载等待时间（秒）</label>
                        <input type="number" id="dy-fire-settings-initial-delay" min="0" max="300" value="${userConfig.initialDelay}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;">
                        <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">打开页面后等待多少秒才开始自动检测和发送（默认30秒，0为立即开始）</div>
                    </div>
                </div>

                <div class="settings-section">
                    <h4 style="color: #33544a; margin: 0 0 15px 0; font-size: 16px; font-weight: 600;">🔄 重试设置</h4>
                    <div style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">最大重试次数</label>
                        <input type="number" id="dy-fire-settings-retry-count" min="1" max="10" value="${userConfig.maxRetryCount}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;">
                    </div>
                    ...
                    <div style="margin-bottom: 15px;">
                        <label style="display: flex; align-items: center; cursor: pointer; margin-bottom: 15px;">
                            ...
                            <span style="color: #7fa197;">达到最大重试次数后继续重试</span>
                        </label>
                        <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">启用后，达到最大重试次数后，会在指定时间后自动重置重试计数并继续尝试</div>
                    </div>
                    
                    <div style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">自动重试间隔（分钟）</label>
                        <input type="number" id="dy-fire-settings-auto-retry-interval" min="1" max="1440" value="${userConfig.autoRetryInterval}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;">
                        <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">达到最大重试次数后，等待多少分钟自动重试</div>
                    </div>
                    
                    <div style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">定时重置重试间隔（分钟）</label>
                        <input type="number" id="dy-fire-settings-retry-reset-interval" min="0" max="1440" value="${userConfig.retryResetInterval}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;">
                        <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">每X分钟自动清除重试计数并尝试发送消息（设置为0禁用）</div>
                    </div>
                    
                    <div style="margin-top: 20px; padding: 15px; background: rgba(255,138,138,0.1); border-radius: 8px; border: 1px solid rgba(255,138,138,0.3);">
                        <div style="font-weight: 600; color: #f27676; margin-bottom: 8px;">⚠️ 注意</div>
                        <div style="font-size: 12px; color: #f27676; line-height: 1.4;">
            ...
            <div id="message-settings" class="settings-tab" style="display: none;">
                <div class="settings-section">
                    <h4 style="color: #33544a; margin: 0 0 15px 0; font-size: 16px; font-weight: 600;">📝 消息内容</h4>
                    <div style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">自定义消息模板</label>
                        <textarea id="dy-fire-settings-custom-message" style="width: 100%; height: 150px; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; resize: vertical; box-sizing: border-box; color: #33544a; font-size: 14px;">${userConfig.customMessage}</textarea>
                        <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">
                            可用占位符:<br>
                            [API] - 一言内容<br>
                            [TXTAPI] - TXTAPI内容<br>
                            [专属一言] - 专属一言内容<br>
                ...
                <div class="settings-section">
                    <h4 style="color: #33544a; margin: 0 0 15px 0; font-size: 16px; font-weight: 600;">💬 Chat页面设置</h4>
                    <div style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">Chat页面换行替换符</label>
                        <input type="text" id="dy-fire-settings-chat-separator" value="${userConfig.chatPageLineSeparator}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;" placeholder="例如:  | ">
                        <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">https://www.douyin.com/chat 页面不支持换行，消息中的换行符将被替换为此分隔符</div>
                    </div>
                </div>
            </div>
            ...
            <div id="api-settings" class="settings-tab" style="display: none;">
                <div class="settings-section">
                    <h4 style="color: #33544a; margin: 0 0 15px 0; font-size: 16px; font-weight: 600;">📚 一言API</h4>
                    <div style="margin-bottom: 15px;">
                        <label style="display: flex; align-items: center; cursor: pointer; margin-bottom: 10px;">
                            <input type="checkbox" id="dy-fire-settings-use-hitokoto" ${userConfig.useHitokoto ? 'checked' : ''} style="margin-right: 10px;">
                            <span style="color: #7fa197;">启用一言API</span>
                        </label>
                    </div>
                   
                    <div style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">一言格式</label>
                        <textarea id="dy-fire-settings-hitokoto-format" style="width: 100%; height: 60px; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; resize: vertical; box-sizing: border-box; color: #33544a; font-size: 14px;">${userConfig.hitokotoFormat}</textarea>
                        <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">
                ...
                <div class="settings-section">
                    <h4 style="color: #33544a; margin: 0 0 15px 0; font-size: 16px; font-weight: 600;">📄 TXTAPI</h4>
                    <div style="margin-bottom: 15px;">
                        <label style="display: flex; align-items: center; cursor: pointer; margin-bottom: 10px;">
                            <input type="checkbox" id="dy-fire-settings-use-txtapi" ${userConfig.useTxtApi ? 'checked' : ''} style="margin-right: 10px;">
                            <span style="color: #7fa197;">启用TXTAPI</span>
                        </label>
                    </div>
                   
                    <div id="txt-api-mode-container" style="margin-bottom: 15px; ${userConfig.useTxtApi ? '' : 'display: none;'}">
                        <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">TXTAPI模式</label>
                        <div style="display: flex; gap: 20px;">
                            <label style="display: flex; align-items: center; cursor: pointer;">
                                <input type="radio" name="txt-api-mode" value="api" ${userConfig.txtApiMode === 'api' ? 'checked' : ''} style="margin-right: 8px;">
                                <span style="color: #7fa197;">API模式</span>
                            </label>
                            <label style="display: flex; align-items: center; cursor: pointer;">
                                <input type="radio" name="txt-api-mode" value="manual" ${userConfig.txtApiMode === 'manual' ? 'checked' : ''} style="margin-right: 8px;">
                                <span style="color: #7fa197;">手动模式</span>
                            </label>
                        </div>
                    </div>
                   
                    <div id="txt-api-url-container" style="margin-bottom: 15px; ${userConfig.useTxtApi && userConfig.txtApiMode === 'api' ? '' : 'display: none;'}">
                        <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">TXTAPI链接</label>
                        <input type="text" id="dy-fire-settings-txtapi-url" value="${userConfig.txtApiUrl}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;" placeholder="例如: https://v1.hitokoto.cn/?encode=text">
                    </div>
                    ...
                    <div id="txt-api-manual-container" style="margin-bottom: 15px; ${userConfig.useTxtApi && userConfig.txtApiMode === 'manual' ? '' : 'display: none;'}">
                        <div style="margin-bottom: 10px;">
                            <label style="display: flex; align-items: center; cursor: pointer;">
                                ...
                                <span style="color: #7fa197;">随机选择文本</span>
                            </label>
                        </div>
                        <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">手动文本内容（一行一个）</label>
                        <textarea id="dy-fire-settings-txtapi-manual" style="width: 100%; height: 120px; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; resize: vertical; box-sizing: border-box; color: #33544a; font-size: 14px;">${userConfig.txtApiManualText}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div id="special-settings" class="settings-tab" style="display: none;">
                <div class="settings-section">
                    <h4 style="color: #33544a; margin: 0 0 15px 0; font-size: 16px; font-weight: 600;">🌟 专属一言设置</h4>
                    <div style="margin-bottom: 15px;">
                        <label style="display: flex; align-items: center; cursor: pointer; margin-bottom: 15px;">
                            ...
                            <span style="color: #7fa197;">启用专属一言</span>
                        </label>
                    </div>
                    
                    <div style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">发送模式</label>
                        <div style="display: flex; gap: 20px;">
                            <label style="display: flex; align-items: center; cursor: pointer;">
                                <input type="radio" name="special-hitokoto-mode" value="random" ${userConfig.specialHitokotoRandom ? 'checked' : ''} style="margin-right: 8px;">
                                <span style="color: #7fa197;">随机发送</span>
                            </label>
                            <label style="display: flex; align-items: center; cursor: pointer;">
                                ...
                                <span style="color: #7fa197;">顺序发送</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                        <div>
                            <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">周一文案（一行一个）</label>
                            <textarea id="dy-fire-settings-special-monday" style="width: 100%; height: 80px; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; resize: vertical; box-sizing: border-box; color: #33544a; font-size: 14px;">${userConfig.specialHitokotoMonday}</textarea>
                        </div>
                        <div>
                            <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">周二文案（一行一个）</label>
                            <textarea id="dy-fire-settings-special-tuesday" style="width: 100%; height: 80px; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; resize: vertical; box-sizing: border-box; color: #33544a; font-size: 14px;">${userConfig.specialHitokotoTuesday}</textarea>
                        </div>
                        <div>
                            <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">周三文案（一行一个）</label>
                            <textarea id="dy-fire-settings-special-wednesday" style="width: 100%; height: 80px; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; resize: vertical; box-sizing: border-box; color: #33544a; font-size: 14px;">${userConfig.specialHitokotoWednesday}</textarea>
                        </div>
                        <div>
                            <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">周四文案（一行一个）</label>
                            <textarea id="dy-fire-settings-special-thursday" style="width: 100%; height: 80px; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; resize: vertical; box-sizing: border-box; color: #33544a; font-size: 14px;">${userConfig.specialHitokotoThursday}</textarea>
                        </div>
                        <div>
                            <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">周五文案（一行一个）</label>
                            <textarea id="dy-fire-settings-special-friday" style="width: 100%; height: 80px; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; resize: vertical; box-sizing: border-box; color: #33544a; font-size: 14px;">${userConfig.specialHitokotoFriday}</textarea>
                        </div>
                        <div>
                            <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">周六文案（一行一个）</label>
                            <textarea id="dy-fire-settings-special-saturday" style="width: 100%; height: 80px; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; resize: vertical; box-sizing: border-box; color: #33544a; font-size: 14px;">${userConfig.specialHitokotoSaturday}</textarea>
                        </div>
                        <div>
                            <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">周日文案（一行一个）</label>
                            <textarea id="dy-fire-settings-special-sunday" style="width: 100%; height: 80px; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; resize: vertical; box-sizing: border-box; color: #33544a; font-size: 14px;">${userConfig.specialHitokotoSunday}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div id="users-settings" class="settings-tab" style="display: none;">
                <div class="settings-section">
                    <h4 style="color: #33544a; margin: 0 0 15px 0; font-size: 16px; font-weight: 600;">👥 用户设置</h4>
                    <div style="margin-bottom: 15px;">
                        <label style="display: flex; align-items: center; cursor: pointer; margin-bottom: 15px;">
                            ...
                            <span style="color: #7fa197;">启用目标用户查找（自动根据目标用户列表状态设置）</span>
                        </label>
                    </div>

                    <div id="target-user-container" style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">目标用户名（一行一个）</label>
                        <textarea id="dy-fire-settings-target-user" style="width: 100%; height: 100px; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; resize: vertical; box-sizing: border-box; color: #33544a; font-size: 14px;" placeholder="每行一个用户名">${userConfig.targetUsernames}</textarea>
                        <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">每行一个用户名，列表不为空时自动启用目标用户查找</div>
                        
                        <div style="margin-top: 15px;">
                            <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">用户发送模式</label>
                            <div style="display: flex; gap: 20px;">
                                <label style="display: flex; align-items: center; cursor: pointer;">
                            ...
                            <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">仅在有多个用户时生效</div>
                        </div>
                        
                        <div style="margin-top: 15px;">
                            <label style="display: flex; align-items: center; cursor: pointer;">
                                <input type="checkbox" id="dy-fire-settings-multi-retry-same" ${userConfig.multiUserRetrySame ? 'checked' : ''} style="margin-right: 8px;">
                                <span style="color: #7fa197;">重试时使用同一用户</span>
                            </label>
                            <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">启用后重试时会继续发送给同一用户，否则会切换到下一用户</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div id="advanced-settings" class="settings-tab" style="display: none;">
                <div class="settings-section">
                    <h4 style="color: #33544a; margin: 0 0 15px 0; font-size: 16px; font-weight: 600;">⚡ 性能设置</h4>
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
                        <div>
                            <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">用户查找超时(毫秒)</label>
                            <input type="number" id="dy-fire-settings-user-timeout" min="1000" max="30000" value="${userConfig.userSearchTimeout}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;">
                        </div>
                        <div>
                            <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">页面加载等待时间(毫秒)</label>
                            <input type="number" id="dy-fire-settings-page-wait" min="1000" max="15000" value="${userConfig.pageLoadWaitTime}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;">
                        </div>
                    </div>

                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
                        <div>
                            <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">查找防抖延迟(毫秒)</label>
                            <input type="number" id="dy-fire-settings-debounce-delay" min="100" max="2000" value="${userConfig.searchDebounceDelay}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;">
                            <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">降低频繁查找导致的性能消耗</div>
                        </div>
                        <div>
                            <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">查找节流延迟(毫秒)</label>
                            <input type="number" id="dy-fire-settings-throttle-delay" min="500" max="3000" value="${userConfig.searchThrottleDelay}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;">
                            <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">控制查找的最小时间间隔</div>
                        </div>
                    </div>
                </div>

                <div class="settings-section">
                    <h4 style="color: #33544a; margin: 0 0 15px 0; font-size: 16px; font-weight: 600;">🔧 技术设置</h4>
                    <div style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">点击方法</label>
                        <div style="display: flex; gap: 20px;">
                            <label style="display: flex; align-items: center; cursor: pointer;">
                                <input type="radio" name="click-method" value="direct" ${userConfig.clickMethod === 'direct' ? 'checked' : ''} style="margin-right: 8px;">
                                <span style="color: #7fa197;">直接点击</span>
                            </label>
                            <label style="display: flex; align-items: center; cursor: pointer;">
                                <input type="radio" name="click-method" value="event" ${userConfig.clickMethod === 'event' ? 'checked' : ''} style="margin-right: 8px;">
                                <span style="color: #7fa197;">事件触发</span>
                            </label>
                        </div>
                        <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">直接点击更可靠，事件触发更安全</div>
                    </div>

                    <div style="margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">最大历史日志数量</label>
                        <input type="number" id="dy-fire-settings-max-logs" min="50" max="1000" value="${userConfig.maxHistoryLogs}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;">
                    </div>
                </div>

                <div class="settings-section">
                    <h4 style="color: #33544a; margin: 0 0 15px 0; font-size: 16px; font-weight: 600;">🎨 格式设置</h4>
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                        <div>
                            <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">from格式</label>
                            <input type="text" id="dy-fire-settings-from-format" value="${userConfig.fromFormat}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;" placeholder="例如: {from}">
                            <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">当from不为空时显示此格式</div>
                        </div>
                        <div>
                            <label style="display: block; margin-bottom: 8px; color: #7fa197; font-weight: 500;">from_who格式</label>
                            <input type="text" id="dy-fire-settings-from-who-format" value="${userConfig.fromWhoFormat}" style="width: 100%; padding: 12px; background: #ffffff; border: 1px solid rgba(46,196,165,0.35); border-radius: 8px; box-sizing: border-box; color: #33544a; font-size: 14px;" placeholder="例如: 「{from_who}」">
                            <div style="font-size: 12px; color: #7fa197; margin-top: 5px;">当from_who不为空时显示此格式</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div style="padding: 20px; border-top: 1px solid ${MINT.cardBorder}; background: rgba(46,196,165,0.06);">
        <button id="dy-fire-settings-save" style="width: 100%; padding: 15px; background: ${MINT.accentGrad}; color: white; border: none; border-radius: 10px; cursor: pointer; font-weight: 600; font-size: 16px; transition: all 0.2s ease; box-shadow: 0 4px 12px ${MINT.shadowAccent};">
            💾 保存设置
        </button>
    </div>
`;
		...
		navItems.forEach(item => {
			...
			item.addEventListener('mouseenter', function() {
				if (!this.classList.contains('active')) {
					this.style.background = 'rgba(46,196,165,0.08)';
				}
			});
		...
		const navStyle = `
            ...
            .settings-section {
                ...
                padding: 20px;
                margin-bottom: 20px;
                border: 1px solid rgba(46,196,165,0.16);
            }
        `;
	...
	function init() {
		...
		if (userConfig.enableTargetUser && allTargetUsers.length > 0) {
			...
			const today = getLocalTodayString();
			if (lastSentDate === today && !alreadyDoneNotified) {
				addHistoryLog('单用户今日发送已完成，启动时确认', 'info');
				checkAllUsersProcessed();
			}
		}
	...
	// 尽早注册消息监听，捕获页面加载时的 user_detail API 请求
	interceptUserDetailApi();

	// 等待存储缓存初始化完成后启动（GM_getValue 依赖内存缓存）
